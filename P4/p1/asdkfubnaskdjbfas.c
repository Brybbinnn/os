...
typedef struct _Queue {
    ...
    /*
    My idea (and sample solution) is based on using one simple queue per priority level.
    In this case these functions need to be extended.
    As I said in the lecture, maybe by just one line
    (or maybe two if you check for invalid priorities to not write out of bounds in an array with queue pointers).
    */
} Queue;
...
static void _enqueueThread(tid_t threadId)
{
    ...
    _enqueue(&priorityQueues, threadId);
}
...
void onThreadReady(int threadId)
{
    ...
    _enqueueThread(threadId); // þarf kannski að vera fyrir neðan assert
    ...
    */
   // IMPORTANT ** APPROPRIATE RUNQUEUE **
}
...
{   for (int i = 0; i < MAX_PRIORITY; ++i) {
        if (priorityQueues[i].head != NULL) {
            return _dequeue(&priorityQueues[i]);
        }
    }
    return -1;
    // TODO: Implement
